...
// All routes below require creator authentication
router.use(verifyCreatorToken);
...
router.get("/analytics/sales", async (req, res) => {
  try {
    const purchases = await db.select().from(purchasesTable).where(eq(purchasesTable.status, "completed"));

    const totalRevenue = purchases.reduce((sum, p) => sum + parseFloat(p.amount as string), 0);
    const totalSales = purchases.length;

    // Group by content
    const byContent: Record<string, { count: number; revenue: number }> = {};
    for (const p of purchases) {
      if (!byContent[p.contentTitle]) {
        byContent[p.contentTitle] = { count: 0, revenue: 0 };
      }
      byContent[p.contentTitle].count++;
      byContent[p.contentTitle].revenue += parseFloat(p.amount as string);
    }

    const contentStats = Object.entries(byContent)
...
async function syncProfileCounts() {
  try {
    ...
    logger.error("Sync profile counts error:", error);
  }
}
